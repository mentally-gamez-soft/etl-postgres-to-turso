...
stop_docker_image () {
   ...
   export APP_NAME="$CURRENT_APP_NAME";
    ...
    # Run the container
    docker compose --env-file Docker/application/${ENVIRONMENT}/app.name.env --env-file Docker/application/${ENVIRONMENT}/app.version.env  -f Docker/application/${ENVIRONMENT}/docker-compose-${ENVIRONMENT}.yaml down 
}
